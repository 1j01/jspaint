...
import { waitForAppLoaded, selectToolByIndex, selectToolByName } from "./utils/test-helpers";
...
test.describe("Magnifier Tool", () => {
	...
	test("magnifier tool changes canvas scale and updates drag handle positions", async ({ page }) => {
		// Step 1: Resize the canvas to a known size
		...
		// Step 2: Verify initial canvas size
		...
		// Canvas internal dimensions should be 400x300
		...
		expect(initialCanvasWidth).toBe(400);
		expect(initialCanvasHeight).toBe(300);

		// Step 3: Get initial drag handle positions (canvas resize handles)
		...
		// Step 4: Select magnifier tool (index 5)
		...
		// Step 5: Get initial CSS transform scale
		...
		// Step 6: Click on canvas with magnifier to zoom in (left click = zoom in)
		...
		// Step 7: Verify canvas is magnified (CSS transform should be scale(2))
		const magnifiedScale = await page.evaluate(() => {
			const canvas = document.querySelector("canvas.main-canvas") as HTMLCanvasElement;
			const transform = window.getComputedStyle(canvas).transform;
			if (transform === 'none') return 1;
			const values = transform.match(/matrix\(([^)]+)\)/)?.[1].split(', ');
		...
		// Step 8: Verify canvas internal dimensions haven't changed
		...
		expect(magnifiedCanvasWidth).toBe(400);
		expect(magnifiedCanvasHeight).toBe(300);

		// Step 9: Verify canvas bounding box has doubled (because of scale(2))
		...
		// Step 10: Verify drag handles have updated their positions
		...
		// Step 11: Click magnifier again to zoom in more (to 4x)
		await canvas.click({
			position: {
				x: canvasBox.width / 2,
				y: canvasBox.height / 2,
		...
		const scale4x = await page.evaluate(() => {
			const canvas = document.querySelector("canvas.main-canvas") as HTMLCanvasElement;
			const transform = window.getComputedStyle(canvas).transform;
			if (transform === 'none') return 1;
			const values = transform.match(/matrix\(([^)]+)\)/)?.[1].split(', ');
		...
		// Step 12: Right-click to zoom out back to 2x
		await canvas.click({
			position: {
				x: canvasBox.width / 2,
				y: canvasBox.height / 2,
		...
		// Step 13: Continue zooming out to 1x
		await canvas.click({
			position: {
				x: canvasBox.width / 2,
				y: canvasBox.height / 2,
		...
		// Step 14: Verify drag handles are back to original positions
