...
test.describe("Edit Colors Dialog", () => {
	...
	test("should display custom colors section", async ({ page }) => {
		...
		const customColors = dialog.locator(
			".custom-colors, [class*='custom'], label:has-text('Custom')",
		);
	...
	test("should have Define Custom Colors button", async ({ page }) => {
		...
		if ((await defineButton.count()) > 0) {
			...
			const colorPicker = dialog.locator(
				'[class*="color-picker"], canvas, input[type="number"]',
			);
	...
	test("should update color preview when selecting basic color", async ({ page }) => {
		...
		const colorCells = dialog.locator(
			".basic-colors .color-cell, .basic-colors button, [class*='color'] button",
		).first();
	...
	test("should cancel without changing color", async ({ page }) => {
		...
		const primaryColorBefore = await page.locator(".primary-color, .foreground-color").first().evaluate((el) => {
		...
		const colorCells = dialog.locator(
			".basic-colors .color-cell, .basic-colors button",
		);
		...
		const primaryColorAfter = await page.locator(".primary-color, .foreground-color").first().evaluate((el) => {
			return window.getComputedStyle(el).backgroundColor;
		});
