	...
	class MultiUserSession {
		...
		constructor(session_id) {
			...
			if (!MultiUserSession.fb_root) {
				$.getScript("lib/firebase.js")  // TODO: remove this, load in html, no lazy
					.done(() => {
						// TODO: Move outide ;-;
						// const config = {
		...
		start() {
			...
			// if (user_id) {
			// 	this.local_user = this.fb_users.child(user_id);
			// }
			// else {
			// 	this.local_user = this.fb_users.push();
			// 	user_id = this.local_user.key;
			// }
			// // Remove the user from the session when they disconnect
			// this.local_user.onDisconnect().remove();
			// // Make the user present in the session
			// this.local_user.set(user);
		...
		_sendCursor(c) {
			log('sending cursor pos ', c)
			this.cursorChannel.send(c);
		}
	}
