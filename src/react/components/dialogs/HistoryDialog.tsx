 ...
 */
export function HistoryDialog({ isOpen, onClose, undoStack, redoStack, onGoToState }: HistoryDialogProps) {
    ...
    return (
      ...
      >
        <div className="history-state-thumbnail inset-deep">
          <img src={dataUrl} alt={t("State {{index}}", { index })} className="history-state-image" />
        </div>
        <div className="history-state-info">
          <div className="history-state-name">{isCurrent ? t("Current State") : t("State {{index}}", { index })}</div>
  ...
  return (
    <Dialog
      ...
      className="dialog-window history-linear-window"
    >
      <div className="history-linear-content">
        <p className="history-linear-status">
          {t("Click on a state to jump to that point in history. Current state:")} <strong>{currentIndex}</strong>{" "}
          {t("of")} <strong>{totalStates - 1}</strong>
        </p>
        ...
        <div className="history-linear-list inset-deep">
          {totalStates === 1 ? (
            ...
            <>
              ...
              {/* Current state */}
              {undoStack.length > 0 && renderStatePreview(undoStack[undoStack.length - 1], currentIndex, true)}

              {/* Redo stack (future states) */}
              {redoStack
                .slice()
                .reverse()
                .map((imageData, i) => renderStatePreview(imageData, currentIndex + 1 + i, false))}
            </>
          )}
        </div>
        ...
        <p className="history-linear-note">
          ...
          {t("This is a simplified linear history view. The full version supports branching history trees.")}
        </p>
