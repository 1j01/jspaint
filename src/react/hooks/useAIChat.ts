...
export function useAIChat(options: UseAIChatOptions): UseAIChatReturn {
	...
	const appendStreamContent = useAIStore(
		(state) => state.appendStreamContent
	);
	...
	const clearPendingCommands = useAIStore(
		(state) => state.clearPendingCommands
	);
	const setExecutionProgress = useAIStore(
		(state) => state.setExecutionProgress
	);
		...
		(content: string) => {
			...
			const callbacks: AIServiceCallbacks = {
				...
				onDone: (message) => {
					...
					const finalContent =
						message ||
						useAIStore.getState().currentStreamContent;
