...
export function exportToGPL(colors: string[], name: string = "Untitled"): string {
	...
	for (const color of colors) {
		...
		if (rgb) {
			lines.push(`${rgb.r.toString().padStart(3)} ${rgb.g.toString().padStart(3)} ${rgb.b.toString().padStart(3)}\tUntitled`);
		}
	}
...
export function importFromGPL(content: string): string[] {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (line.startsWith("#") || line === "" || line.startsWith("GIMP") || line.startsWith("Name:") || line.startsWith("Columns:")) {
			startIndex = i + 1;
		} else {
			break;
		}
