...
function decodeBMP(arrayBuffer) {
	...
	return {
		bitsPerPixel: decoder.bitPP,
		colorTable: decoder.palette.map(({red, green, blue})=> ({r: red, g: green, b: blue})),
		imageData: new ImageData(new Uint8ClampedArray(decoder.data), Math.abs(decoder.width), Math.abs(decoder.height)),
	};
}
...
class BmpDecoder {
	...
	parseHeader() {
		...
		this.reserved1 = this.view.getUint16(this.pos, true);
		this.pos += 2;
		this.reserved2 = this.view.getUint16(this.pos, true);
		this.pos += 2;
		this.offset = this.readUint32();
		// End of BITMAP_FILE_HEADER
		this.headerSize = this.readUint32();
		if (!(this.headerSize in HeaderTypes)) {
			throw new Error(`Unsupported BMP header size ${this.headerSize}`);
		}
		// @FIXME: these are supposed to be LONGs, and negative height determines pixel row order
		...
		this.pos += 2;
		this.compression = this.readUint32();
		this.rawSize = this.readUint32();
		this.hr = this.readUint32();
		this.vr = this.readUint32();
		this.colors = this.readUint32();
		...
		if (this.headerSize > HeaderTypes.BITMAP_INFO_HEADER ||
			...
			this.maskRed = this.readUint32();
			this.maskGreen = this.readUint32();
			this.maskBlue = this.readUint32();
		}
		// End of BITMAP_V2_INFO_HEADER
		if (this.headerSize > HeaderTypes.BITMAP_V2_INFO_HEADER ||
			this.compression === 6 /* BI_ALPHA_BIT_FIELDS */) {
			this.maskAlpha = this.readUint32();
		}
	...
	parseRGBA() {
		this.data = new DataView(new ArrayBuffer(this.width * this.height * 4));
		switch (this.bitPP) {
			case 1:
				this.bit1();
				break;
	...
	bit16() {
		...
		this.scanImage(padding, this.width, (x, line) => {
			...
			const px = this.view.getUint16(this.pos, true);
			this.pos += 2;
