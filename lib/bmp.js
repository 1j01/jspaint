...
function decodeBMP(arrayBuffer) {
	new BmpDecoder(arrayBuffer, {toRGBA: true});
}
...
class BmpDecoder {
	constructor(arrayBuffer, { toRGBA } = { toRGBA: false }) {
		...
		this.pos = 0;
		this.bottomUp = true;
		this.flag = this.view.toString('utf-8', 0, (this.pos += 2));
		if (this.flag !== 'BM') {
	...
	parseHeader() {
		...
		this.reserved1 = this.view.getUInt16LE(this.pos, true);
		this.pos += 2;
		this.reserved2 = this.view.getUInt16LE(this.pos, true);
		this.pos += 2;
		...
		this.planes = this.view.getUInt16LE(this.pos, true);
		this.pos += 2;
		this.bitPP = this.view.getUInt16LE(this.pos, true);
		this.pos += 2;
	...
	bit16() {
		...
		this.scanImage(padding, this.width, (x, line) => {
			...
			const px = this.view.getUInt16LE(this.pos, true);
			this.pos += 2;
	...
	readUInt32LE() {
		const value = this.view.getUInt32LE(this.pos, true);
		this.pos += 4;
		return value;
	}
