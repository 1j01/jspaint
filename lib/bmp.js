...
function encodeBMP(imgData, bitsPerPixel = 24, palette) {
	...
	if (indexed) {
		...
		// }
		const res = UPNG.quantize(imgData.data, colorCount);
		indices = res.inds;
		rgb_table = res.plte.map((color_entry)=> color_entry.est.q.map((component)=> Math.round(component * 255)));
	}
	if (rgb_table) {
		for (const [r, g, b] of rgb_table) {
