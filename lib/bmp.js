...
function decodeBMP(arrayBuffer) {
	function readBitmapFileHeader(view) {
		return {
			filesize: view.getInt32(2, true),
			imageDataOffset: view.getInt32(10, true)
		};
	}
	...
	function readDibHeader(view) {
		const dibHeaderLength = view.getInt32(14, true);
		const header = {};
		...
		header.height = view.getInt32(22, true);
		if (header.headerType == "BITMAPCOREHEADER") {
			return header;
		}
		header.bitsPerPixel = view.getInt16(28, true);
		header.compressionType = view.getInt32(30, true);
		if (header.headerType == "OS22XBITMAPHEADER") {
			return header;
		}
		header.bitmapDataSize = view.getInt32(34, true);
		header.numberOfColorsInPalette = view.getInt32(46, true);
		header.numberOfImportantColors = view.getInt32(50, true);
	...
	function readColorTable(view) {
		...
		const sourceStart = 14 + dibHeader.headerLength; // @TODO: use sourceStart? test with different BMP versions
		view.copy(colorTable, 0, 54, 54 + colorTable.length);
		return colorTable;
	}
